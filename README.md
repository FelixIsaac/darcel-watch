...
Sample: 160 random resource IDs from the AskDarcel API (`askdarcel.org/api`), seed 7, IDs 1–2600.

| Metric | Count |
|---|---|
| IDs sampled | 160 |
| Resolved to real records | 122 |
| Marked `status: approved` (live to users) | 108 |
| Of those, `verified_at: null` (never verified) | 98 |
| Of those, carry a verification date | 10 |
| Median age of the 10 dated verifications | ~2,837 days (~7.8 years) |
| Verified in the past year | 0 |
| Of the 108, list a website we can check | 93 |
...
- **`graph.py`** — models the directory as a graph: `org` nodes linked to `service`, `address`, `phone`, and `category` nodes. From the 122 sampled records this builds a graph of **949 nodes** (122 org, 265 service, 246 category, 161 address, 155 phone). Three things depend on this and don't exist without it:
  - `blast_radius(org)` — if an org is wrong, what else is now suspect: every service it offers, plus any other org sharing its address or phone. One traversal. In our run, the largest blast radius found was **36** — one organisation whose closure would invalidate 36 downstream services plus co-located orgs.
  - `contradictions()` — distinct orgs sharing the same phone or address: merge candidates or stale data. A graph pattern match, not a table scan. Our run found **7** of these. The clearest one: phone 415-831-2700 is shared by "Miraloma Playground Clubhouse", "Civic Center Plaza", and "Laurel Hill Playground and Clubhouse" — three separate listings routing through one switchboard. Exactly the pattern a flat table hides and a graph surfaces.
...
- **`verify.py`** — for each candidate record with a website: fetches the homepage, and if the evidence doesn't settle the question, walks a fixed list of likely sub-pages (`/contact`, `/about`, `/hours`, `/visit`, `/locations`) up to 3 fetches total, stopping early once it has enough signal (a closure phrase, or a matching phone + hours). It checks phone number, address, and closure/relocation language against what's stored. Findings are handed to an adjudicator:
  - **With `GEMINI_API_KEY`**: Gemini 2.5 Flash judges whether the scraped quote actually contradicts the stored value and writes the one-sentence human-readable reason. It's instructed to abstain when the quote doesn't clearly settle the question.
...
# (omit GEMINI_API_KEY to use the deterministic fallback adjudicator)
...
- Sample size is 160 IDs out of a much larger corpus — a snapshot, not a full audit.
